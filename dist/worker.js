    ...
    try {
      ...
      if (!upgradeHeader || upgradeHeader !== "websocket") {
        switch (url.pathname) {
          ...
          default:
            return new Response("This site can\u2019t provide a secure connection", {
              status: 404,
              headers: {
...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, log) {
  ...
  async function retry() {
    let jsonLandingAddr = parselandingAddr(landingAddress);
    let host = jsonLandingAddr.host || addressRemote;
    let port = jsonLandingAddr.port || portRemote;
